...
class ECPoint:
    ...
    def __add__(self, other: 'ECPoint'):
        ...
        xr = (d**2 - self.x - other.x) % self.curve.p # xr = delta^2 - xp - xq
        yr = (d * (self.x - xr) - self.y) % self.curve.p # yr = delta(xp - xr) - yp
    ...
    def __mul__(self, scalar):
        ...
        if scalar < 0: # P * (-k) = (-P) * k 
            return (-self) * abs(scalar) # Return the negative point multiplied by positive scalar

        # Double-and-add method, similar to square-and-multiply in RSA
        sum = ECPoint(None, None, self.curve)  # Accumulator for the result, start with point at infinity O
        addend = self # Start with the original point P

        while scalar:
            if scalar & 1:  # Do bitwise AND to 000...1 to check if the least significant bit is 1
                sum = sum + addend
            
            addend = addend + addend  # Point doubling
            scalar >>= 1  # Shift right by one bit, equivalent to integer division by 2
    ...
    def __repr__(self):
        ...
        if self.is_infinity():
            return f"ECPoint(None, None, curve={self.curve})"
        return f"ECPoint({self.x}, {self.y}, curve={self.curve})"
...
class EllipticCurve:
    ...
    def order(self, generator):
        """Calculate the order of the elliptic curve group generated by the given point"""
        ...
        # Iteratively add the generator to itself until reaching the point at infinity
        while True:
            current = current + generator
            order += 1
            
            # Found the point at infinity
            if current.is_infinity():
                break
                
            # Safety check to prevent infinite loops
            if order > 10000:  
                raise ValueError(f"Order exceeds safety limit (10000), point may not generate a cyclic subgroup")
        
        return order

    def is_on_curve(self, x, y):
        """Check whether (x, y) lies on the curve"""
        LHS = (y**2) % self.p # Left Hand Side
        RHS = (x**3 + self.a * x + self.b) % self.p # Right Hand Side
        return LHS == RHS
    ...
    def __eq__(self, other):
        """Check curve equality"""
        return self.a == other.a and self.b == other.b and self.p == other.p

    def __str__(self):
        """String representation of the curve"""
        return f"y^2 = x^3{f' + {'' if self.a == 1 else self.a}x' if self.a else ''}{f' + {self.b}' if self.b else ''} (mod {self.p})"
    
    def __repr__(self):
        """Return an unambiguous representation for debugging or reconstruction."""
